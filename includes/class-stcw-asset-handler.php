...
class STCW_Asset_Handler {
    ...
    public function queue_asset_downloads($assets) {
        ...
        stcw_log_debug('Queueing ' . count($assets) . ' assets');
        
        // Merge with existing pending assets
        $existing = get_option('stcw_pending_assets', []);
        $merged = array_unique(array_merge($existing, $assets));
        update_option('stcw_pending_assets', $merged, false);
        
        // Schedule background processing if not already scheduled
        if (!wp_next_scheduled('stcw_process_assets')) {
            wp_schedule_single_event(time() + 10, 'stcw_process_assets');
        }
    }
    ...
    public function download_queued_assets() {
        ...
        if (empty($assets)) {
            ...
            update_option('stcw_pending_assets', array_values($assets), false);
    ...
    public function ajax_process_pending() {
        check_ajax_referer('stcw_process', 'nonce');
    ...
    public function download_to_assets($url, $retry = 0) {
        ...
        $downloaded = get_option('stcw_downloaded_assets', []);
        ...
        if (!file_exists($dest)) {
            ...
            if (is_wp_error($response)) {
                ...
                stcw_log_debug('Failed to download: ' . $url);
                return false;
            }
            ...
            if ($code < 200 || $code > 299) {
                stcw_log_debug('HTTP ' . $code . ' for: ' . $url);
                return false;
            }
            ...
            if ($wp_filesystem) {
                ...
                if ($result === false) {
                    stcw_log_debug('Failed to save asset: ' . $filename);
                    return false;
                } else {
                    stcw_log_debug('Successfully saved asset: ' . $filename);
                }
            } else {
                stcw_log_debug('Failed to initialize WP_Filesystem for saving asset: ' . $filename);
                return false;
            }
